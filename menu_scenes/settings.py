import pygame, menu_scenes.levels
...
class XButton():
    ...
    class Button():
        ...
        def drav(self, screen, size):
            ...
            pygame.draw.rect(screen, self.color, self.rect, int(size))
    ...
    def drav(self, screen):
        vidth, height = screen.get_size()
        pygame.draw.rect(screen, self.back, self.rect)
        pygame.draw.rect(screen, self.color, self.rect, int(max(vidth, height)*0.003))#*----------------------------------
        ...
        if self.q_vis:
            ...
            pygame.draw.rect(screen, (255,255,255), self.q_box, int(height*0.01))
            self.yes_btn.drav(screen, height*0.01)
            self.no_btn.drav(screen, height*0.01)
...
def drav_settings(screen):
    ...
    if not int(stats[1][:-1]) + int(stats[2][:-1]) == 0:
